...
export async function sendPhoneCode(input: string): Promise<boolean> {
  const formatted = normalizeIL(input);
  if (!formatted) {
    alert('Please enter a valid Israeli phone (e.g. 054-9203398 or "+972549203398").');
    return false;
  }
  ...
  try {
    ...
    if (code === "auth/operation-not-allowed") {
      alert(
        "Phone sign-in is not fully enabled for this project.\n\n" +
          "Do this:\n" +
          "1) Auth → Sign-in method → Phone = Enabled\n" +
          "2) Add an Authorized domain: localhost\n" +
          "3) Add a Test phone number (e.g. +972549203398 with code 123456)\n" +
          "Then restart your dev server."
      );
    } else if (code === "auth/too-many-requests") {
      alert("Too many attempts. Please wait a minute and try again.");
    } else if (code === "auth/invalid-phone-number") {
      alert("Invalid phone number format. Try +972549203398.");
    } else {
      console.error("sendPhoneCode error:", e);
      alert("Failed to send SMS. Please check your setup or try again later.");
    }
    return false;
  }
}
...
export async function confirmPhoneCode(code: string) {
  if (!confirmationResult) {
    alert("No SMS verification in progress.");
    return;
  }
  try {
